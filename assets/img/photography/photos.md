# Photography manifest

Edit this file to manage the gallery. **One row = one photo.** Row order = display order.

To add a photo: drop the full-size original `<file>.jpg` into this folder, then add a
row below. Push to GitHub — web-sized copies are generated automatically. No HTML to edit.

- **file** — the image's base filename, no extension (e.g. `DSCF1234`).
- **place** — where it was taken (proper nouns; shown as-is in every language).
- **year** — when it was taken.
- **caption** — one sentence describing the photo. Used as the lightbox caption and as
  the image's alt text for accessibility. Keep it factual and language-neutral.

| file     | place              | year | caption                                                              |
|----------|-----------------------|------|----------------------------------------------------------------------|
| DSCF2858 | Lapinha da Serra, MG  | Jul 2025 | Lapinha da Serra, MG – Jul 2025 |
| DSCF1312 | São Paulo | Jan 2025 | São Paulo – Jan 2025               |
| DSCF2762 | Inhotim     | Jul 2025 | Inhotim – Jul 2025 |
| DSCF1284 | São Paulo          | Jan 2025 | São Paulo – Jan 2025     |
| DSCF2803 | Inhotim           | Jul 2025 | Inhotim – Jul 2025         |
| DSCF2996 | Lapinha da Serra, MG     | 2025 | Lapinha da Serra, MG – Jul 2025 |
| DSCF2697 | Inhotim            | Jul 2025 | Inhotim – Jul 2025 |
| DSCF3174 | Corcovado, RJ            | Jul 2025 | Corcovado, RJ – Jul 2025 |
| DSCF1313 | São Paulo        | 2025 | A man in a wide hat pauses below a mountain in the Brazilian highlands |
| DSCF5727 | Bogotá            | Apr 2026 | Bogotá – 2026 |

